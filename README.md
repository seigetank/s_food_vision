...
- 사전학습 `yolov8n.pt`와 COCO8 미세조정 모델을 같은 검증셋에서 비교하고 Precision, Recall, mAP50, mAP50-95를 저장합니다.
...
- 평가 결과와 그래프는 `outputs/week3_evaluation/`에 저장합니다.
...
사전학습 모델과 COCO8 미세조정 모델을 같은 검증셋에서 비교합니다. 평가 JSON, 비교 그래프, PR curve, 정규화 confusion matrix는 `outputs/week3_evaluation/`에 저장합니다.

현재 재현 결과:

- Precision: `0.6210` → `0.7600`
- Recall: `0.8333` → `0.8333`
- mAP50: `0.8875` → `0.8890`
- mAP50-95: `0.6291` → `0.6244`

Precision은 증가했지만 mAP50-95는 `0.0046` 감소했습니다. 학습 이미지가 4장뿐이므로 미세조정이 일반적인 성능 개선으로 이어졌다고 볼 수 없습니다.
...
Depth Map 및 3D 좌표 배열 생성 함수와 3주차 평가 지표 변환·비교 함수를 검증합니다.

## Unit Test

테스트 파일은 `tests/test_depth_processing.py`, `tests/test_week3_evaluation.py`입니다.
...
- `test_metrics_to_dict`
  - Ultralytics 평가 객체에서 Precision, Recall, mAP 지표를 추출하는지 확인합니다.
- `test_compare_metrics_calculates_deltas`
  - baseline과 미세조정 모델 사이의 지표 변화를 계산하는지 확인합니다.
...
- `test_metrics_to_dict`: PASSED
- `test_compare_metrics_calculates_deltas`: PASSED
- 총 5개 테스트 통과
...
![YOLO metric comparison](outputs/week3_evaluation/metrics_comparison.png)

![YOLO PR curve](outputs/week3_evaluation/BoxPR_curve.png)
